...
-- Fixes: missing counselor_weekly_availability + legacy faculty names
-- Safe to run more than once (skips steps already applied).

-- ── 1) Rename legacy enum values only when 'faculty' still exists ───────────
DO $$
BEGIN
  IF EXISTS (
...
-- ── 2) Rename legacy tables / columns ─────────────────────────────────────
DO $$ BEGIN
...
-- ── 3) Create weekly availability table if still missing ────────────────────
