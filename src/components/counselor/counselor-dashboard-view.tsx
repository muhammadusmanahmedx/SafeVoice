...
import { CounselorCharts } from "@/components/counselor/counselor-charts";
...
export function CounselorDashboardView({
  ...
  typeBreakdown,
  highRiskCases,
}: CounselorDashboardViewProps) {
  const { t } = useLanguage();

  const stats = [
    { labelKey: "counselor.dashboard.stats.openCases", subKey: "counselor.dashboard.stats.awaitingReview", value: openCases, icon: Bell, iconBg: "bg-blue-50", iconColor: "text-blue-600", href: "/counselor/alerts" },
    { labelKey: "counselor.dashboard.stats.inProgress", subKey: "counselor.dashboard.stats.beingManaged", value: activeCases, icon: Clock, iconBg: "bg-amber-50", iconColor: "text-amber-600", href: "/counselor/cases" },
    { labelKey: "counselor.dashboard.stats.resolved", subKey: "counselor.dashboard.stats.successfullyClosed", value: resolvedCases, icon: CheckCircle2, iconBg: "bg-green-50", iconColor: "text-green-600", href: "/counselor/cases" },
    { labelKey: "counselor.dashboard.stats.highRisk", subKey: "counselor.dashboard.stats.urgentReview", value: highRiskCount, icon: AlertTriangle, iconBg: "bg-red-50", iconColor: "text-red-600", href: "/counselor/alerts" },
  ];
  ...
  return (
    <div className="space-y-7 pb-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-[#193852]">{t("counselor.dashboard.title")}</h1>
          <p className="mt-0.5 text-sm text-gray-500">{t("counselor.dashboard.subtitle")}</p>
        </div>
        <Link href="/counselor/alerts"
        ...
        >
          <Bell className="h-4 w-4" /> {t("counselor.dashboard.reviewAlerts")}
        </Link>
      </div>
        ...
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-extrabold uppercase tracking-wider text-[#193852]">
              {t("counselor.dashboard.highPriorityAlerts")}
            </h2>
          </div>
          <Link href="/counselor/alerts" className="text-xs font-semibold text-[#a38016] hover:underline">
        ...
        <div className="divide-y divide-gray-50">
          ...
          {highRiskCases.length === 0 && (
            <div className="flex flex-col items-center py-12 text-center">
              ...
              <p className="text-sm font-semibold text-gray-600">{t("counselor.dashboard.noHighRiskTitle")}</p>
              <p className="mt-1 text-xs text-gray-400">{t("counselor.dashboard.noHighRiskDesc")}</p>
            </div>
          )}
        </div>
      </div>
