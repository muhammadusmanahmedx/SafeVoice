...
interface CounselorCaseDetailProps {
  caseData: {
    id: string;
    status: CaseStatus;
    summary: string;
...
export function CounselorCaseDetail({ caseData, messages, notes, studentInfo, revealRequests = [] }: CounselorCaseDetailProps) {
      ...
      <div className="space-y-5 lg:col-span-2">
        {caseData.auto_alerted && (
          <div className="flex items-start gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4">
            ...
            <div>
              <p className="text-sm font-semibold text-amber-700">{t("counselor.caseDetail.autoAlertTitle")}</p>
              <p className="mt-0.5 text-xs text-amber-600">{t("counselor.caseDetail.autoAlertDesc")}</p>
            </div>
          </div>
        )}
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {formatMessage(t("counselor.caseDetail.submitted"), { date: formatDate(caseData.created_at) })}
              </p>
            </div>
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold">{t("counselor.caseDetail.anonymousMessages")}</h3>
          </div>
          <div className="space-y-3">
            {messages.length === 0 && (
              <p className="text-sm text-muted-foreground">{t("counselor.caseDetail.noMessages")}</p>
            )}
            {messages.map((msg) => (
              <div key={msg.id}
                className={cn(
                  "rounded-xl px-4 py-3 text-sm",
                  msg.sender_role === "counselor" ? "ml-8 bg-primary/10" : "mr-8 bg-muted"
                )}
              >
          ...
          <div className="mt-4 space-y-2">
            <Textarea
              ...
              placeholder={t("counselor.caseDetail.responsePlaceholder")}
              rows={3}
              className="resize-none text-sm"
            />
            <Button size="sm" onClick={handleRespond} disabled={loading || !response.trim()}>
              {t("counselor.caseDetail.sendResponse")}
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="mb-3 flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold">{t("counselor.caseDetail.studentIdentity")}</h3>
          </div>
          ...
          {!identityKnown && !revealRequested && (
            <>
              <div className="flex items-center gap-2 text-sm text-muted-foreground mb-3">
                ...
                <span>{t("counselor.caseDetail.identityAnonymous")}</span>
              </div>
              <Button variant="outline" size="sm" className="w-full" onClick={handleRequestReveal} disabled={loading}>
                {t("counselor.caseDetail.requestReveal")}
              </Button>
              <p className="mt-2 text-xs text-muted-foreground">{t("counselor.caseDetail.revealConsentHint")}</p>
            </>
          )}
            ...
            <div className="rounded-lg bg-green-500/10 border border-green-500/20 p-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-green-600">{t("counselor.caseDetail.identitySharedConsent")}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {!identityKnown && revealRequested && revealStatus === "pending" && (
            <div className="rounded-lg bg-amber-500/10 border border-amber-500/20 p-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-medium text-amber-700">{t("counselor.caseDetail.requestSent")}</p>
              </div>
              <p className="mt-1 text-xs text-amber-600">{t("counselor.caseDetail.requestSentDesc")}</p>
              {latestReveal && (
                <p className="mt-1.5 text-[10px] text-muted-foreground">
                  {formatMessage(t("counselor.caseDetail.requested"), { date: formatDate(latestReveal.created_at) })}
                </p>
              )}
            </div>
          )}
          ...
          {!identityKnown && revealRequested && revealStatus === "accepted" && (
            <div className="rounded-lg bg-green-500/10 border border-green-500/20 p-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-medium text-green-700">{t("counselor.caseDetail.identityShared")}</p>
              </div>
              <p className="mt-1 text-xs text-green-600">{t("counselor.caseDetail.identitySharedDesc")}</p>
              {latestReveal?.responded_at && (
                <p className="mt-1.5 text-[10px] text-muted-foreground">
                  {formatMessage(t("counselor.caseDetail.accepted"), { date: formatDate(latestReveal.responded_at) })}
                </p>
              )}
            </div>
          )}
          ...
          {!identityKnown && revealRequested && revealStatus === "declined" && (
            <div className="rounded-lg bg-destructive/10 border border-destructive/20 p-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-medium text-destructive">{t("counselor.caseDetail.requestDeclined")}</p>
              </div>
              <p className="mt-1 text-xs text-destructive/80">{t("counselor.caseDetail.requestDeclinedDesc")}</p>
              {latestReveal?.responded_at && (
                <p className="mt-1.5 text-[10px] text-muted-foreground">
                  {formatMessage(t("counselor.caseDetail.declined"), { date: formatDate(latestReveal.responded_at) })}
                </p>
              )}
            </div>
          )}
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="mb-3 flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold">{t("counselor.caseDetail.internalNotes")}</h3>
          </div>
          <div className="space-y-2 mb-3">
            {notes.length === 0 && (
              <p className="text-xs text-muted-foreground">{t("counselor.caseDetail.noNotes")}</p>
            )}
            {notes.map((n) => (
