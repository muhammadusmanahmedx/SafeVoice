...
interface CounselorChartsProps {
  severityData: { key: string; value: number; color: string }[];
  weeklyTrend: { day: string; count: number }[];
  typeBreakdown: { type: string; count: number }[];
}

export function CounselorCharts({ severityData, weeklyTrend, typeBreakdown }: CounselorChartsProps) {
  const { t } = useLanguage();
  ...
  const CustomTooltip = ({ active, payload, label }: { active?: boolean; payload?: { value: number }[]; label?: string }) => {
    if (active && payload && payload.length) {
      ...
      const casesLabel = count !== 1
        ? formatMessage(t("counselor.dashboard.charts.casesTooltipPlural"), { count })
        : formatMessage(t("counselor.dashboard.charts.casesTooltip"), { count });
      return (
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-3">
      <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm lg:col-span-2">
        <p className="mb-1 text-xs font-bold uppercase tracking-wider text-gray-400">{t("counselor.dashboard.charts.weeklyActivity")}</p>
        <p className="text-sm font-extrabold text-[#193852]">{t("counselor.dashboard.charts.casesLast7Days")}</p>
        <div className="mt-4">
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
        <p className="mb-1 text-xs font-bold uppercase tracking-wider text-gray-400">{t("counselor.dashboard.charts.riskBreakdown")}</p>
        <p className="text-sm font-extrabold text-[#193852]">{t("counselor.dashboard.charts.severityDistribution")}</p>
        <div className="mt-2">
          <ResponsiveContainer width="100%" height={160}>
            <PieChart>
              ...
              <Tooltip formatter={(val, name) => [
                formatMessage(t("counselor.dashboard.charts.casesTooltipPlural"), { count: Number(val) }),
                name,
              ]} />
            </PieChart>
          </ResponsiveContainer>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm lg:col-span-3">
        <p className="mb-1 text-xs font-bold uppercase tracking-wider text-gray-400">{t("counselor.dashboard.charts.incidentTypes")}</p>
        <p className="text-sm font-extrabold text-[#193852]">{t("counselor.dashboard.charts.mostCommonCategories")}</p>
        <div className="mt-4">
          <ResponsiveContainer width="100%" height={160}>
