...
interface FacultyCasesViewProps {
  cases: {
    id: string;
    status: CaseStatus;
...
export function FacultyCasesView({ cases }: FacultyCasesViewProps) {
  const { t } = useLanguage();

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">{t("faculty.cases.pageTitle")}</h1>
        <p className="text-sm text-muted-foreground">{t("faculty.cases.pageSubtitle")}</p>
      </div>
