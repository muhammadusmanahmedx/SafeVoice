...
interface FacultyAlertsViewProps {
  cases: {
    id: string;
    status: CaseStatus;
...
export function FacultyAlertsView({ cases }: FacultyAlertsViewProps) {
  const { t } = useLanguage();

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">{t("faculty.alerts.pageTitle")}</h1>
        <p className="text-sm text-muted-foreground">{t("faculty.alerts.pageSubtitle")}</p>
      </div>
      ...
      <div className="space-y-3">
        ...
        {cases.length === 0 && (
          <p className="text-sm text-muted-foreground">{t("faculty.alerts.empty")}</p>
        )}
      </div>
    </div>
  );
