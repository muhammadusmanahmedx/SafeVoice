...
interface FacultyCounselingViewProps {
  upcomingBookings: {
    id: string;
    topic: string | null;
    studentName: string;
...
export function FacultyCounselingView({
  upcomingBookings,
  weeklyRows,
  initialDuration,
  timeZone,
}: FacultyCounselingViewProps) {
  const { t } = useLanguage();

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">{t("faculty.counseling.pageTitle")}</h1>
        <p className="text-sm text-muted-foreground">{t("faculty.counseling.pageSubtitle")}</p>
      </div>

      <Tabs defaultValue="bookings">
        <TabsList>
          <TabsTrigger value="bookings">
            {t("faculty.counseling.studentBookings")}
          ...
          <TabsTrigger value="availability">{t("faculty.counseling.weeklyHours")}</TabsTrigger>
        </TabsList>

        <TabsContent value="bookings" className="mt-6 space-y-3">
          {upcomingBookings.length === 0 ? (
            <p className="text-sm text-muted-foreground">{t("faculty.counseling.noUpcoming")}</p>
          ) : (
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      {!joinable && (
                        <p className="mt-1 text-xs text-muted-foreground">
                          {formatMessage(t("faculty.counseling.joinAvailableOn"), {
